...
partition by RANGE_BUCKET(doc_id_partition, GENERATE_ARRAY(0, 99, 1)) as
select distinct * from (
  SELECT
    etb.*,
    'ANY' as pref_name,
    to_hex(md5(concat('ANY', ':', etb.standard_type, ':', etb.radius))) as chessleague_uuid,
    mod(doc_id_greatest, 100) as doc_id_partition
...
EXPORT DATA OPTIONS(
  uri='gs://wizepair2_batch/elo_input/elo_input-*.csv.gz',
  format='CSV',
  overwrite=true,
  header=true,
  field_delimiter=',',
